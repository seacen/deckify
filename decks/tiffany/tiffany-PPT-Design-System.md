# Tiffany & Co.-PPT-Design-System

> 为 Tiffany & Co. 制作的所有 deck 共用的视觉语言。请严格遵循，让每一份新 deck 都一眼被识别为同一家族。
...
****studio luxury restraint** — 借鉴 1960–80 年代 Vogue 编辑式排版与 Hermès 印刷物的视觉语言：极致克制的 Didone 衬线大字、慷慨的留白、Tiffany Blue (PMS 1837) 作为唯一商标级和弦、其余皆为黑墨与暖纸。每一条 hairline 都有功能；每一寸留白都是奢侈。所有元素都是 sharp（radius:0）、flat（无阴影）、archival（编辑式而非促销式）。禁止：装饰渐变、stock photography、SaaS 风圆角、emoji、彩色矩阵 — 任何会让 Tiffany 显得像快时尚或仪表盘的元素。**
...
- Cormorant Garamond typeface, no serif/display fonts （仅 Cormorant Garamond + 系统 Didone fallback；不混入 Inter、Helvetica、SF Pro 这类 humanist sans）
- 12 px 可读性下限
- 每一张 slide 都必须有 logo
...
**Bespoke 元素（鼓励）：**
- **在调色板内自由发挥。** Tiffany Blue 横条作为顶部 promo banner、Didone 大字 wordmark 作为封面唯一锚点、暖纸（#FBF9F7）上一条 hairline rule（#C1C1C1）分隔编辑式段落、纯黑等距按钮 (radius:0、padding:32×22) 作为 CTA、section eyebrow 用 13 px 全大写字距 .24em — 都属于「系统内 bespoke」，不是组件库里的具名物。
...
:root {
  ...
  --accent:   #009CAD;   /* CTA / link / single saturated highlight */
  /* ── Neutrals ── */
  --surface:  #FBF9F7;   /* Paper / slide bg */
  --white:    #FFFFFF;
  --ink:      #000000;   /* Body text on light surfaces */
  --mid:      #565656;   /* Secondary text / muted labels */
  --rule:     #C1C1C1;   /* Dividers / hairlines */
  --tint:     #F6F6F4;   /* Subtle row / section bg */
  /* ── Semantic (invariant names; values may map to brand-palette colours) ── */
  --green:    #346E4A;   /* Positive */
  --green-bg: #E8F2EC;
  --red:      #A02835;   /* Negative */
  --red-bg:   #F5E6E8;
  --warn:     #A07020;   /* Warning / caution */
  --warn-bg:  #F5EFDF;
  --teal:     #009CAD;   /* Informational / neutral highlight */
  --teal-bg:  #E0F4F6;
  /* ── Brand palette (brand-specific names; expanded from brand.json accents+neutrals) ── */
  --tiffany-blue:   #009CAD;   /* PMS 1837 — registered trademark color, used as the brand's signature chord */
  --tiffany-paper:  #FBF9F7;   /* warm off-white, computed from real footer bg — Tiffany's paper ground */
  --tiffany-ink:    #000000;   /* pure black, never softened — used for body type and CTA fills */
  --tiffany-rule:   #C1C1C1;   /* 1px hairline divider, frequency 25 in CSS */
  --tiffany-mid:    #565656;   /* secondary text gray for editorial paragraphs */
  --tiffany-tint:   #F6F6F4;   /* subtle row bg, 0.6% darker than surface */
  --tiffany-gold:   #A07020;   /* 18K gold accent — used sparingly, e.g. metal-grade indicator */
  --tiffany-deep:   #003D3F;   /* deep teal, 65% darker Tiffany Blue, for rare dark-mode covers only */
}
```
...
**Cormorant Garamond** — 唯一字体。字重 300 / 400 / 500 / 700（300 italic 用于封面副标的克制斜体）。`Bodoni 72 → Didot → Georgia` 作为 macOS / iOS 系统 Didone 兜底；Windows / Linux 端落到 Georgia 时仍保留高对比度衬线骨架。

> Tiffany 的字体语言是**单一 Didone 家族**贯穿所有层级：封面、内页大标、副标、引用、表格全部用 Cormorant Garamond + Bodoni/Didot fallback。正文段落（≥ 16 px）也用 Cormorant Garamond — 高对比 hairline 在长段落里依然清晰，反而强化了 archival 编辑感。**不要混入 humanist sans**（Inter、SF Pro、Helvetica）—— 那会立刻把 Tiffany 拉成 SaaS。**全大写字距 +0.18~0.24em** 用于 wordmark + nav + eyebrow + 表头 — 这是 Tiffany 的 nav 语言。
...
## 4. Tiffany & Co. Logo <!-- BRAND-VARIABLE: SVG payload is brand-specific; surrounding pattern + multi-format support is ENGINEERING-DNA -->
...
<svg style="display:none" aria-hidden="true">
  <symbol id="brand-wm" viewBox="0 0 634 70" fill="currentColor">
    <path fill-rule="evenodd" clip-rule="evenodd" d="M523.185 70C533.491 70 543.663 65.4493 547.812 62.1033L549.284 47.2466H547.678C544.198 65.0478 531.216 67.7247 523.72 67.7247C509.533 67.7247 500.699 56.2141 500.699 36.5392C500.699 13.1166 510.068 3.21223 523.453 3.21223C535.231 3.21223 543.931 12.3136 547.009 22.7534H548.615L547.009 0H545.403C545.001 4.0153 543.529 5.21988 541.388 5.21988C537.372 5.21988 532.42 0.936899 522.65 0.936899C504.848 0.936899 489.59 16.0612 489.59 36.5392C489.456 54.2065 503.51 70 523.185 70Z"/>
...
<path fill-rule="evenodd" clip-rule="evenodd" d="M437.392 66.1185C427.755 66.1185 422.669 59.4263 422.669 51.3957C422.669 44.7036 424.944 38.5468 432.038 34.7992L450.91 59.2925C446.359 64.1108 441.808 66.1185 437.392 66.1185ZM436.053 26.367C432.841 22.2179 429.896 17.8011 429.896 12.5812C429.896 7.49514 433.242 2.94448 438.864 2.94448C444.887 2.94448 448.367 8.16437 448.367 14.7227C448.367 20.0764 446.894 25.8317 438.596 29.7131L436.053 26.367ZM432.841 69.9999C439.399 69.9999 446.091 67.7246 452.516 61.1663L453.453 62.237C455.46 64.6462 457.067 67.3231 457.87 68.7953H479.82V67.0554C472.057 67.0554 467.506 65.8508 458.003 54.0726C469.915 36.2714 471.388 34.933 477.946 34.933V33.193H454.925V34.933C459.877 34.933 461.885 36.8068 461.885 40.0191C461.885 43.0974 460.011 47.3804 456.665 52.3326L439.935 31.4531C451.98 25.9655 454.657 19.1395 454.657 14.455C454.657 6.55823 448.233 0.802979 439.265 0.802979C429.629 0.802979 422.936 8.69975 422.936 16.8642C422.936 21.8164 425.212 26.0994 430.03 32.1223L430.833 33.0592C416.512 40.1529 413.701 48.4512 413.701 54.2064C413.701 64.9139 424.275 69.9999 432.841 69.9999Z"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M589.839 67.8585C578.462 67.8585 567.888 57.6864 567.888 35.4684C567.888 13.2505 578.462 3.07835 589.839 3.07835C601.215 3.07835 611.789 13.3843 611.789 35.4684C611.655 57.6864 601.215 67.8585 589.839 67.8585ZM589.839 70C608.309 70 623.032 54.4742 623.032 35.3346C623.032 16.3288 608.309 0.669189 589.839 0.669189C571.368 0.669189 556.646 16.3288 556.646 35.3346C556.646 54.4742 571.368 70 589.839 70Z"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M43.6252 68.7955V67.0555H41.7514C35.8623 67.0555 35.0592 65.7171 35.0592 58.8911V4.14917H38.1376C50.1835 4.14917 55.5372 7.76293 58.7495 15.6597H60.3556L57.1434 2.00769H3.20454L-0.00769043 15.6597H1.59842C4.81066 7.76293 10.1644 4.14917 22.2103 4.14917H25.2887V58.8911C25.2887 65.7171 24.4856 67.0555 18.5965 67.0555H16.7227V68.7955H43.6252Z"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M93.5492 68.7955V67.0555H92.4785C86.5894 67.0555 85.7863 65.7171 85.7863 58.8911V11.9121C85.7863 5.21992 86.5894 3.74764 92.4785 3.74764H93.5492V2.00769H68.119V3.74764H69.1898C75.0789 3.74764 75.8819 5.08608 75.8819 11.9121V58.8911C75.8819 65.7171 75.0789 67.0555 69.1898 67.0555H68.119V68.7955H93.5492Z"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M132.764 68.7955V67.0555H129.686C124.198 67.0555 123.395 65.7171 123.395 58.8911V34.5316H128.214C138.386 34.5316 142.535 36.9408 142.803 45.5067H144.409V21.9503H142.803C142.535 31.1855 136.512 32.6578 128.214 32.6578H123.395V4.01534H129.552C143.472 4.01534 149.762 5.75529 153.242 15.7935H154.848L152.038 2.00769H105.862V3.74764H107.468C112.956 3.74764 113.892 5.08608 113.892 11.9121V58.8911C113.892 65.7171 113.089 67.0555 107.468 67.0555H105.862V68.7955H132.764Z"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M187.373 68.7955V67.0555H184.295C178.807 67.0555 178.004 65.7171 178.004 58.8911V34.5316H182.823C192.995 34.5316 197.144 36.9408 197.412 45.5067H199.018V21.9503H197.412C197.144 31.1855 191.121 32.6578 182.823 32.6578H178.004V4.01534H184.161C198.081 4.01534 204.371 5.75529 207.851 15.7935H209.457L206.647 2.00769H160.471V3.74764H162.077C167.565 3.74764 168.501 5.08608 168.501 11.9121V58.8911C168.501 65.7171 167.698 67.0555 162.077 67.0555H160.471V68.7955H187.373Z"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M329.381 69.5985V11.2428C329.381 6.29061 331.656 3.61375 335.806 3.61375V1.87378H318.674V3.61375H320.414C324.563 3.61375 326.838 6.29061 326.838 11.2428V51.9311L289.228 1.87378H270.892V3.61375C274.907 3.61375 278.922 4.68449 282.67 9.63669V59.4264C282.67 64.3786 280.394 67.0555 276.245 67.0555V68.7954H293.779V67.0555H291.503C287.354 67.0555 285.079 64.3786 285.079 59.4264V12.4474L328.043 69.4646H329.381V69.5985Z"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M387.469 68.7954V67.0555H384.658C379.706 67.0555 378.903 66.2524 378.903 61.3002V37.0745L393.224 14.8566C399.782 4.6845 402.058 3.61375 408.214 3.61375V1.87378H384.792V3.61375C390.279 3.61375 392.421 5.08601 392.421 8.43209C392.421 10.9751 390.949 13.652 388.941 16.7304L377.297 35.2007L362.306 10.0382C361.236 8.29826 360.7 7.09367 360.7 6.02293C360.7 4.6845 361.503 3.74758 364.448 3.74758H367.258V2.00763H340.891V3.74758C346.111 3.74758 348.119 4.14911 351.465 9.77052L369.132 39.0822V61.434C369.132 66.3862 368.329 67.1893 363.377 67.1893H360.566V68.9293H387.469V68.7954Z"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M628.253 69.0631C631.465 69.0631 634.008 66.5201 634.008 63.3079C634.008 60.0956 631.465 57.5526 628.253 57.5526C625.041 57.5526 622.498 60.0956 622.498 63.3079C622.364 66.5201 625.041 69.0631 628.253 69.0631Z"/>  <!-- inner <path>/<g> carry no explicit fill, so currentColor cascades in -->
  </symbol>
</svg>
```
...
<svg style="display:none" xmlns="http://www.w3.org/2000/svg"
  ...
  <symbol id="brand-wm" viewBox="0 0 634 70">
    <image href="data:image/png;base64,{{LOGO_BASE64}}" width="634" height="70"/>
  </symbol>
</svg>
```
...
```html
<!-- 白色（深色 slide 上） -->
<svg class="logo W" viewBox="0 0 634 70" aria-label="Tiffany & Co.">
  <use href="#brand-wm"/>
</svg>

<!-- 品牌深色（浅色 slide 上） -->
<svg class="logo L" viewBox="0 0 634 70" aria-label="Tiffany & Co.">
  <use href="#brand-wm"/>
</svg>
```
...
.logo   { height: 26px; width: auto; flex-shrink: 0; fill: currentColor; }
.logo.W { color: #fff; }
...
- Logo 周围最小留白 = logo 高度（26px）四周都留。
- 不要拉伸、不要在 `W`/`L` 之外重新着色、不要把 logo 叠在带图案的区域上。

**Tiffany & Co.® 商标使用限制**：Tiffany 的 wordmark + Tiffany Blue + Tiffany Blue Box 都是注册商标。本 DS 只用于内部 / 受邀演示；商业对外发布或与第三方品牌联名传播必须先取得 Tiffany 法务批准。永远不要：(a) 把 Tiffany wordmark 拉伸 / 倾斜 / 嵌入复合 logo；(b) 用任何非官方 Tiffany Blue (PMS 1837) 的近似蓝；(c) 把 wordmark 叠在繁忙图像或非中性背景上。
...
#### Quiet / silent shell 也必须遵守（Tiffany 专属补充）

Tiffany 的封面与 pull-quote 页（§6 Type A、Type J）会用一个去掉左侧 `--accent` 边线的 quiet 变体 — 视觉上比标准 `.sw` 更安静，但**结构上仍必须使用 `.sw .sc` shell + 1 个 absorber**。`fit_contract_intact` hard check 是按 `.sw .sc` 内 child 的 `flex-grow` 计数的；任何另起 `class="pullquote-page"` / `class="cov-shell"` 这类自定义容器都会被判定 `absorbers=0` 而 fail。

**正确实现**（Tiffany pull-quote 页）：

```html
<section class="slide sw silent" data-num="08">
  <div class="sc">
    <div class="pq-page-eyebrow">Brand Voice · Closing</div>          <!-- flex: 0 0 auto -->
    <div class="pq-rule-top"></div>                                   <!-- flex: 0 0 auto -->
    <div class="pq-page-body">…大引述 + 署名…</div>                    <!-- flex: 1 1 0; min-height: 0; overflow: hidden  ← absorber -->
    <div class="pq-page-coda">…SOURCE + logo + 页码…</div>             <!-- flex: 0 0 auto -->
  </div>
</section>
```

```css
.sw.silent           { border-left: none; }   /* 去掉 --accent 左线，保留 .sw 的纸面背景 + flex 列结构 */
.sw.silent .sc       { padding: 0; }          /* 让内嵌的 eyebrow / body / coda 各自管理自己的水平内边距 */
.pq-page-body        { flex: 1 1 0; min-height: 0; overflow: hidden; ... }   /* ← 唯一 absorber */
.pq-page-eyebrow,
.pq-rule-top,
.pq-page-coda        { flex: 0 0 auto; }
```

**为什么不能另起 shell**：fit-contract 与 mobile media query（§10）都把 `.sw` 当作"内容页 shell"的标识，用它做 selector hook。绕开它意味着重写一遍移动端折叠规则、字体安全检查、scale-to-fit 测量边界 — 全部隐性破坏。`.sw.silent` 是"换皮不换骨"的正确做法。

**Cover (Type A) 例外**：`.cov` 是封面专属 shell，hard check 已显式跳过（`check_fit_contract` 跳 slide 0）。所以封面**不需要** `.sw .sc` 与 absorber 规则；其它所有内容页（包括 pull-quote）都必须遵守。
...
> **Emphasis for Tiffany & Co.**: Tiffany 的品牌叙事最强烈地体现在 **A 封面**（Didone 大字 + Tiffany Blue chord）、**J 引述**（「Stories. Since 1837.」这类 archival epigraph）、**K 时间线**（1837 → today 的 188 年 heritage 叙事）、**F 图像页**（lifestyle 摄影 + 极简 caption 的编辑式排版）。
> Foreground these types when designing decks: **A 封面**、**J 引述**、**K 时间线**、**F 图像页**.
> Use sparingly: Type G（交互演示）和 Type I（多 tab）— Tiffany 的氛围是 archival、慢节奏、看一遍就懂；多步交互会破坏奢侈品的从容感。需要时可少量使用，但不应是 Tiffany deck 的主调。.

### Type A — 封面
- 背景：`var(--surface)（暖纸 #FBF9F7）— Tiffany 封面用纸面而非 Tiffany Blue 满底；Tiffany Blue 仅作为顶部一条 promo bar 或单条 hairline rule 出现。封面文字（Didone wordmark）是黑墨`
...
### Type D — 翻面卡片
两张卡片并排。正面 = `--primary`，背面 = `var(--ink)（纯黑）`（比 `--accent` 柔和）。**Hover + 点击翻面** — JS `onclick` 切换 `.on` class（移动端必需）。正面有 ghost 罗马数字。背面留白宽（32 px padding，≤ 4 个内容元素）。
...
- 标题陈述洞察，不是图表类型。好："Tiffany & Co. 在三项维度都领先"。坏："柱状图对比"。
...
**Tiffany 实现规范（archival pull-quote）**：Tiffany 的 J 页用作 deck 收尾的 archival epigraph（如 "Stories. Since 1837."），刻意使用 quiet shell，去掉 `.sw` 默认的 `--accent` 左侧边线 — Tiffany 的 voice 是克制的，不要任何抢戏的 chrome 元素。同时大引述字号开到 56–64 px（远超模板基础值 28–36 px）以支撑封面到收尾的视觉重量呼应；引述内可用 italic + `var(--accent)` 着色单个语义词（如 "*Since*"）作为唯一 chord。**仍必须使用 `.sw .sc` shell + 1 个 absorber**（详见 §5.1 Quiet/silent shell 章节）。规范类名：`.sw.silent`。
...
.tb { padding: 7px 16px; border: 1px solid var(--rule); background: transparent; font: 800 12px/1 'Cormorant Garamond'; letter-spacing: .06em; color: var(--mid); cursor: pointer; }
...
Tiffany 的图像语言是 **archival lifestyle 摄影** — 慢门、自然光、温暖肤色、近距离工艺细节、复古黑白档案照。**绝不**：装饰渐变、stock photo、AI 生成占位、彩色矩阵图示。图像要么传达工艺（手作环、抛光面、宝石切工细节），要么传达场景（约会、晨光、家庭瞬间），要么传达档案（1837 创始海格里夫文献、Audrey Hepburn 早餐场景）。处理：`border-radius: 0`（Tiffany 不用圆角）、可选 `1px solid var(--rule)` 暖灰边框；图说用 `.cap` 全大写字距样式作为编辑式 caption。
...
| 卡片间距 | 20 px |
| 卡片内边距 | 32 px |
| 圆角 | 0 px（sharp，整本 DS 不存在圆角 — Tiffany 的 visual 语言里柔圆 = 廉价） |
| 分隔线粗细 | 1 px |
| Accent 边线 | 3 px |
...
- [ ] 不用 emoji (👍🎉 等) — 排版符号 (✓ − ! ×) 可用
- [ ] Cormorant Garamond 300 / 400 / 500 / 700 + 300 italic 已加载；不用 serif / display 字体
- [ ] 封面副标：仅用 Cormorant Garamond 300 斜体（如果 300 italic 不可用，用品牌等价值）
