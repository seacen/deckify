...
**Procter & Gamble is a 185-year-old consumer-goods conglomerate, and the deck system reflects that institutional weight. The mood is corporate-heritage, type-led, understated authority — a board pack, not an ad. Source-Sans-grade typography sits over a warm cream surface; navy and corporate blue do all the dramatic work; whitespace is generous and decoration is absent. There are no gradients, no campaign exclamation, no stock imagery, no decorative motifs. Every element earns its place by carrying information, not by performing.**

> Mood reference: corporate-heritage / institutional / understated authority. Closer to "annual report" than "consumer marketing." If a slide reads as exuberant, it has drifted off-brand — pull it back toward type-led restraint.
...
- Source Sans 3 typeface, no serif/display fonts; Inter as system fallback only
- 12px readability floor
- Logo on every slide
- No emoji (👍🎉 etc.) — typographic symbols (✓ − ! ×) and geometric indicators are permitted
- No decorative stock imagery
- No decorative gradients of any kind — solid fills only
...
- **Invent freely** within the colour palette. A hand-built portfolio grid (CSS-tiled brand mnemonics), a CSS-drawn margin-contribution bar chart, a custom timeline for a multi-quarter outlook — all in-system, none of them named in §7.
...
:root {
  /* ── Brand ── */
  --navy:     #002A6E;   /* Primary brand dark — P&G institutional */
  --blue:     #003DA5;   /* Accent / CTA — P&G corporate disc-monogram blue */
  /* ── Neutrals ── */
  --surface:  #F7F4EE;   /* Off-white slide bg — warm cream-tinged */
  --white:    #FFFFFF;
  --ink:      #1B1F2A;   /* Body text on light — near-black, cool undertone */
  --mid:      #5C6573;   /* Secondary text */
  --rule:     #DAD5CB;   /* Dividers — warm-leaning */
  --tint:     #EFEAE0;   /* Subtle separation bg */
  /* ── Semantic ── */
  --green:    #2C6E49;   /* Positive */
  --green-bg: #E8F0EA;
  --red:      #A03026;   /* Negative */
  --red-bg:   #F4E7E5;
  --warn:     #B07810;   /* Warning / caution */
  --warn-bg:  #F8F0DD;
  --teal:     #1F5D6E;   /* Informational / neutral highlight */
  --teal-bg:  #E1EEF1;
}
```
...
- **No gradients, ever.** P&G's institutional register avoids gradient fills — flat colour only.

---

## 3. Typography

**Source Sans 3** — sole typeface. Weights 300–900, italic 300 reserved for cover subtitle only. `Inter` system fallback for environments where Source Sans is not licensed/available.

> Typography does the editorial heavy lifting. P&G is type-led: the page voice is set by Source Sans 3 weight contrast (300 vs 900) and tight tracking on titles, not by colour or imagery. Italic 300 is rare, reserved for cover subtitles to mark the editorial register.
...
P&G's saved asset on this workspace is a 32×32 PNG favicon — not usable as a vector logo at deck scale. We substitute a **typographic monogram** built as an inline `<symbol>`: a filled disc plus the wordmark "P&G". Both shapes inherit `currentColor` from the outer `<svg>`, so the same symbol flips white-on-navy / navy-on-cream cleanly via the `.W` and `.L` modifier classes.

> **Production note:** swap this typographic placeholder for the licensed P&G corporate logo SVG once available. Keep the `<symbol id="brand-wm">` ID and viewBox so all `<use>` references continue to work without per-slide edits.

```html
<svg style="display:none" aria-hidden="true">
  <symbol id="brand-wm" viewBox="0 0 80 32">
    <circle cx="16" cy="16" r="14"/>
    <text x="38" y="23" font-family="Source Sans 3, Inter, sans-serif"
          font-weight="900" font-size="18" letter-spacing="-0.02em"
          fill="#FFFFFF">P&amp;G</text>
  </symbol>
</svg>
```

The `<circle>` has no explicit fill, so it picks up `currentColor` from the outer `<svg>`. The `<text>` is hardcoded white so the wordmark always reads against the disc, regardless of which variant (`.W` or `.L`) is in use — i.e., on `.L` slides the disc is navy with white wordmark; on `.W` slides the disc is white with white wordmark (effectively a wordmark-only mark on dark backgrounds, which is fine because the surrounding surface is navy).

### Usage

```html
<!-- White (on dark slides) -->
<svg class="logo W" viewBox="0 0 80 32" aria-label="Procter & Gamble">
  <use href="#brand-wm"/>
</svg>

<!-- Brand-dark (on light slides) -->
<svg class="logo L" viewBox="0 0 80 32" aria-label="Procter & Gamble">
  <use href="#brand-wm"/>
</svg>
```
...
- Minimum clear space around the logo = logo height (26px) on all sides.
- Never stretch, recolour outside `W`/`L`, or overlay the logo on a patterned area.

**Brand restrictions:** the corporate P&G logo is licensed; this typographic monogram is a placeholder only. Do not distribute decks externally with the placeholder — replace with the licensed corporate SVG before any external share.
...
4. **Write the copy short enough that single-line bullets don't wrap**. In a half-width column at 13 px, budget ~36 characters per bullet before wrapping.
...
> **Emphasis for Procter & Gamble**: this is board-pack content. Foreground quantitative and structured slide types; soften the more performative ones.
> Foreground these types when designing decks: Type E (data table — board content), Type H (chart — quantitative), Type C (full-width narrative).
> Use sparingly: Type D (flip cards), Type G (interactive demos).

### Type A — Cover
- Background: solid `var(--navy)` — no gradient, no image, no decorative line.
...
### Type D — Flip cards
Two cards side-by-side. Front = `--navy`, back = `#013685` (one shade lighter than `--blue`, softer for the back face). **Hover + click flip** — JS `onclick` toggles `.on` class (required for mobile). Ghost Roman numerals on front. Spacious back (32px padding, ≤ 4 content elements).
...
Slide dominated by a table or structured data grid. Used for brand-portfolio summaries, P&L breakdowns, specification matrices. The table component spec (§7.7) defines the element-level design; this type defines when to use it and how to lay out the slide around it.

**Principles:** The table is the star — title + table + optional one-line callout below. No side panels competing for attention. If the table has 6+ columns, let it span full width. **This is the workhorse type for P&G board content.**

### Type F — Image slide
One or more images dominate the slide, with text anchored to a calm area. For P&G, "image" usually means a CSS-drawn portfolio grid or schematic — not a stock photo.

**Principles:**
- Images must serve comprehension — no decorative stock. Prefer: product UI screenshots, real data visualizations, contextual photos that illustrate a specific point, CSS-drawn brand grids.
- Image treatment: `border-radius: 3px`, optional `1px solid var(--rule)` border. On dark backgrounds, no border needed.
...
A self-contained, click-to-advance micro-experience embedded in a slide. **Use sparingly for P&G** — board contexts rarely need this. When used: clean typography, restrained animation, max 5 steps, 4px-radius dark "screen" area.
...
- Title states the insight, not the chart type. Good: "Premium tier delivered 73% of margin growth on 22% of volume". Bad: "Bar Chart Comparison".
...
A single striking statement that anchors a narrative moment. Used for board-council quotes, audience reframes, or memorable one-liners.

**Structure:** Large quote text (32px, weight 700, `--ink`) centered or left-aligned. Optional attribution below (14px, `--mid`). Left border accent (`3px solid var(--blue)`).

### Type K — Timeline / roadmap
Horizontal sequence of milestones. Used for quarterly project plans, multi-year evolution narratives, phase descriptions. Component spec in §7.12.
...
.tb { padding: 7px 16px; border: 1px solid var(--rule); background: transparent; font: 800 12px/1 'Source Sans 3'; letter-spacing: .06em; color: var(--mid); cursor: pointer; }
...
P&G is a board-pack register. Imagery is functional, not decorative — CSS-drawn portfolio grids, real chart visualizations, occasional product shots when discussing a specific brand. Stock photography is forbidden. Hero gradients and abstract decoration are forbidden. If the slide doesn't *need* an image to be understood, it shouldn't have one.
...
- **Contextual photography**: Rarely. When a scenario benefits from visual grounding, search for and include a relevant image (e.g., shelf scenarios, product packaging).
...
2. **CSS-drawn alternatives**: Bar charts, progress bars, timeline diagrams, portfolio grids — preferable to external images when data is simple.
...
- `border-radius: 3px`. Optional `1px solid var(--rule)` border on light backgrounds.
...
1. **Flip cards for reveals**: thesis/reality, plan/result, before/after.
2. **Concrete over abstract**: specific brands and deltas beat generic descriptions.
3. **Visual evidence**: Charts > text. Tables > prose. Diagrams > bullet lists.
...
- [ ] Source Sans 3 weights 300–900 loaded; Inter fallback declared; no serif or display fonts
- [ ] Cover subtitle: Source Sans 3 300 italic only
