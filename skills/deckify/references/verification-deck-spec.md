...
| `logo_renders` (typographic placeholder, path `d` < 40 chars, no `<image href>`, or `<symbol>` inner element has any hardcoded `fill` — including `fill="none"` — overriding `currentColor`) | DS §4 Logo (fill-cascade pitfall) | The `<symbol id="brand-wm">` must contain either a real `<path>` with `d` ≥ 40 chars OR an `<image href="data:...">`. Outer SVG carries `fill: currentColor`; **NO inner element** carries a `fill` attribute (not even `fill="none"` on a wrapper `<g>`). If `embed_logo.py` produced an `embed.html` that still has `fill="none"`, re-pull from upstream — that bug was fixed; older artefacts may carry stale strip rules. If you hand-pasted a logo, manually delete every inner `fill="..."` before saving. |
| Logo "embedded" but invisible on screenshots, while `logo_renders` says PASS | DS §4 + `evals/hard_checks.py` `hasInnerFill` selector | Open `<brand>/source/assets/logo.embed.html` and grep for ` fill="`. If anything other than `fill="currentColor"` (on the outer `<symbol>`) is present, that is the bug. The `embed_logo.py` strip regex must drop **every** inner `fill`; the `hasInnerFill` selector must include `fill="none"`. Both fixes shipped after the Mars end-to-end test — verify your skill checkout includes them (`grep 'fill="none"' scripts/embed_logo.py evals/hard_checks.py` should match a comment, not a live exception). |
